...
const { upperSnakeCase } = require('./helpers/upper-snake-case');
...
module.exports = class SecretHandler {
	...
	async getValue(fullValueData = false) {
		...
		try {
			...
			const valuePromise = isLocalEnv() ? this.getLocalSecret() : AWS.secretsManager.getSecretValue(params);
	...
	getLocalSecret() {
		const secretName = upperSnakeCase(this.secretName);
		return process.env[`AWS_SM_${secretName}`];
	}
